...
export class EncriptadoService {
  ...
  encriptarInformacionRutas(valor: string): any {
    ...
    if (existeValor) {
      const encrypted = crypto.AES.encrypt(crypto.enc.Utf8.parse(valor), AES_RUTA, CONFIG_AES_RUTAS);
  ...
  encriptarInformacionCookie(valor: string): any {
    const existeValor = valor && valor !== null;
    if (existeValor) {
      const encrypted = crypto.AES.encrypt(crypto.enc.Utf8.parse(valor), AES_COOKIE, CONFIG_AES_COOKIE);
      return encrypted.toString();
    }
  }
  ...
  desencriptarInformacionCookie(valor: string): any {
    const existeValor = valor && valor != null;
    if (existeValor) {
      const decrypted = crypto.AES.decrypt(valor, AES_COOKIE, CONFIG_AES_COOKIE);
  ...
  encriptarInformacionRequest(valor: any): any {
    const existeValor = valor && valor != null;
    if (existeValor) {
      const encrypted = crypto.AES.encrypt(crypto.enc.Utf8.parse(valor), AES_REQUEST,
        CONFIG_AES_REQUEST);
      return encrypted.toString();
    }
  }
  ...
  desencriptarInformacionResponse(valor: any): any {
    const existeValor = valor && valor != null;
    if (existeValor) {
      const decrypted = crypto.AES.decrypt(valor, AES_RESPONSE, CONFIG_AES_RESPONSE);
      return decrypted.toString(crypto.enc.Utf8);
    }
  }
